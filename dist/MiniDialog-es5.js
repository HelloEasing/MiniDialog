...
!function (global, factory) {
	...
	var $$ = function $$(elem, context) {
		var arrayLike = (typeof elem === "undefined" ? "undefined" : _typeof(elem)) === "object" ? elem.length ? elem : [elem] : (context || document).querySelectorAll(elem);
	...
	var Dialog = function Dialog() {
		...
		if (Fn.isPlainObject(opt.iframeContent)) {
			...
			$main.innerHTML = "<iframe src=\"" + src + "\" frameborder=\"0\" scrolling=\"auto\"></iframe>";
		}
		...
		if (Fn.isPlainObject(videoContent)) {
			...
			$main.innerHTML = "<video src=\"" + _src2 + "\" width=\"" + dialogWidth + "\" height=\"" + _height2 + "\" controls></video>";
			if (autoplay) {
		...
		if (Fn.isPlainObject(opt.SHORTCUTS) && !Fn.isEmptyObject(opt.SHORTCUTS)) {
			...
			$wrapper.style.width = (lowerWidth ? w - 60 : 420) + "px";
	...
	Dialog.success = function (title, content) {
		DialogShortcuts("success", title, content);return Dialog;
	};
	Dialog.warn = function (title, content) {
		DialogShortcuts("warn", title, content);return Dialog;
	};
